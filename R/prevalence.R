...
#' \strong{Methods:}
#'
#' The \code{survey} package is used to compute all estimates.
#' Confidence intervals are computed with a significance level of \code{0.05}.
#' \link[survey]{svymean} is used the z-score mean and CIs, while for the
#' prevalence estimates \link[survey]{svyciprop} is used with the \code{logit}
#' method.
#'
#' For cases where \code{cluster/strata/sw is NULL} a fast approximation
#' is used that does not use the \code{survey} package but computes the same
...
anthro_prevalence <- function(
  ...
  input[["age_in_days"]] <- age_to_days(
    input[["age"]],
    is_age_in_month = is_age_in_month
  )
  input[["age_in_months"]] <- age_to_months(
    input[["age"]],
    is_age_in_month = is_age_in_month
  )
  ...
  filter_zscores <- is.na(input[["age_in_days"]]) |
    input[["age_in_days"]] <= 1826
  ...
  if (!all(valid_wealthq_values)) {
    warning(
      "Some entries in wealthq are out of range. Allowed values are: ",
  ...
  input[["age_sex_interaction"]] <- interaction(
    input[["age_group"]],
    input[["sex"]]
  )
  ...
  if (all(included_strata == FALSE)) {
    stop(
      "It seems that all values have been removed from the analysis, ",
  ...
  compute_prevalence_of_zscores(
    ...
    zscores_to_compute = list(
      list(
        ...
        column = "len",
        with_cutoffs = TRUE,
        with_auxiliary_zscore_column = FALSE
      ),
      list(
        name = "WA",
        column = "wei",
        with_cutoffs = TRUE,
        with_auxiliary_zscore_column = TRUE
      ),
      list(
        name = "BMI",
        column = "bmi",
...
prevalence_significance_level <- 0.05
...
compute_prevalence_of_zscores <- function(
  data,
  zscores_to_compute,
  survey_subsets
) {
  ...
  if (
    !is.null(data[["sampling_weights"]]) &&
      any(data[["sampling_weights"]] < 0, na.rm = TRUE)
  ) {
    stop(
  ...
  zscores_to_compute <- lapply(zscores_to_compute, function(indicator) {
    if (isTRUE(indicator$with_cutoffs)) {
      indicator$cutoffs <- generate_cutoffs(prev_prevalence_column_name(
        indicator
      ))
    }
    indicator
  ...
  survey_design <- build_design(res)
...
build_design <- function(survey_data) {
  ...
  if (isTRUE(getOption("anthro.internal.compute_method") == "survey")) {
    return(build_survey_design(survey_data))
  }
  is_simple_problem <- is.null(survey_data[["cluster"]]) &&
    is.null(survey_data[["strata"]]) &&
    (is.null(survey_data[["sampling_weights"]]) ||
...
compute_prevalence_stunting_wasting <- function(
  survey_design,
  subset_col_name
) {
  indicator <- list(
  ...
  list(
    ...
    compute_prevalence_estimates_for_column(
      ...
      indicator$name,
      subset_col_name,
      prev_prevalence_column_name(indicator)
    )
  )
}

compute_prevalence_stunting_overweight <- function(
  survey_design,
  subset_col_name
) {
  indicator <- list(
...
compute_prevalence_for_each_subset_and_indicator <- function(
  subset_col_names,
  subset_labels,
  survey_design,
  indicators
...
create_zscore_auxiliary_columns <- function(zscores_to_compute, dataframe) {
  ...
  if (has_zlen_and_wfl) {
    zlen <- dataframe[["zlen"]]
    zwfl_aux <- dataframe[["zwfl_aux"]]
    is_na <- is.na(zlen) | is.na(zwfl_aux)
    res <- rep.int(0L, length(zlen))
    res[is_na] <- NA_integer_
    make_combined_aux_col <- function(flag_fun) {
      res_local <- res
      res_local[zlen < -2 & flag_fun(zwfl_aux)] <- 1L
      res_local
    }
...
compute_prevalence_sample_size <- function(
  survey_design,
  indicator,
  subset_col_name
) {
  res <- compute_prevalence_sample_size_by(
    survey_design,
    indicator,
    subset_col_name
  )
  ...
  if (indicator_name == "HA_2_WH2") {
    col_idx_to_drop <- which(
      colnames(res) %in% c("HA_2_WH2_pop", "HA_2_WH2_unwpop")
    )
...
compute_prevalence_cutoff_summaries <- function(
  survey_design,
  indicator,
  subset_col_name
) {
...
compute_prevalence_estimates_for_column <- function(
  survey_design,
  indicator_name,
  subset_col_name,
  prev_col_name
) {
  all_na <- all(is.na(column_values(survey_design, prev_col_name)))
  res <- if (all_na) {
    data.frame(
      Group = unique_groups(column_values(survey_design, subset_col_name)),
      r = NA_real_,
      se = NA_real_,
      ll = NA_real_,
      ul = NA_real_,
    ...
    compute_prevalence_estimates_for_column_by(
      survey_design,
      indicator_name,
      subset_col_name,
      prev_col_name
...
compute_prevalence_zscore_summaries <- function(
  survey_design,
  indicator,
  subset_col_name
) {
  zscore_col_name <- prev_zscore_value_column(indicator)
  all_na <- all(is.na(column_values(survey_design, zscore_col_name)))
  res <- if (all_na) {
    data.frame(
      Group = unique_groups(column_values(survey_design, subset_col_name)),
      r = NA_real_,
    ...
    compute_prevalence_zscore_summaries_by(
      survey_design,
      indicator,
      subset_col_name
    )
...
build_simple_design <- function(survey_data) {
  structure(
...
build_survey_design <- function(survey_data) {
  ...
  just_one_cluster <- has_cluster_info &&
    length(unique(survey_data[["cluster"]])) == 1L
